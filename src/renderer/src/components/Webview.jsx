...
export default memo(function Webview({ account }) {
  ...
  const { title, partition, proxy } = account
  ...
  useEffect(() => {
    ...
    window.electron.ipcRenderer
      .invoke('setup-session', { partition, proxy, extensionPath })
      .then((extension) => {
        webview.src = extension.url + 'index.html'
      })
  ...
  }, [partition, proxy, extensionPath])

  return (
    <div key={partition} className={cn('grow flex flex-col shrink-0')}>
      <div className="flex items-center gap-2 p-1">
        {/* Title */}
        <div className="flex font-bold grow text-blue-500">{title}</div>
